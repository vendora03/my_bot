...
def init_db():
    ...
    DB_Create_Table_User(conn) 
    DB_Create_Table_Variable(conn)
    DB_Create_Table_Schedule(conn)
    DB_Create_Table_Template(conn)
    DB_Create_Table_VIP_Code(conn)
    DB_Create_Table_VIP_Variable(conn)
    DB_Create_Table_Settings(conn)
...
def DB_Create_Table_User(connect = None):
    if not connect:
        conn = DB_Get_Connection()
    else:
        conn = connect
...
# ====== [DB] Drop Table User =========== 
def DB_Drop_Table_User(connect = None):
    if not connect:
        conn = DB_Get_Connection()
    else:
        conn = connect
...
# ====== [DB] Create Table Variable ===== 
def DB_Create_Table_Variable(connect = None):
    if not connect:
        conn = DB_Get_Connection()
    else:
        conn = connect
...
# ====== [DB] Insert All Variable ======= 
def DB_Save_All_Variable(data, connect = None):
    if not connect:
        conn = DB_Get_Connection()
    else:
        conn = connect
...
# ====== [DB] Get Variable ==============
def DB_Get_Variable(access_code: str):
    # if DEBUG:
...
# ====== [DB] Get All Variable ==========
def DB_Get_All_Variable():
    # if DEBUG:
    #     print("[DB] Get All Variable")
...
# ====== [DB] Drop Table Variable ======= 
def DB_Drop_Table_Variable(connect = None):
    if not connect:
        conn = DB_Get_Connection()
    else:
        conn = connect
...
def DB_Create_Table_Schedule(connect = None):
    if not connect:
        conn = DB_Get_Connection()
    else:
        conn = connect
...
# ====== [DB] Drop Table Schedule ======= 
def DB_Drop_Table_Schedule(connect = None):
    if not connect:
        conn = DB_Get_Connection()
    else:
        conn = connect
    cursor = conn.cursor()
    cursor.execute("DROP TABLE IF EXISTS daily_schedule")
    if not connect:
        conn.commit()
        conn.close()
...
# ====== [DB] Create Table Template ===== 
def DB_Create_Table_Template(connect = None):
    if not connect:
        conn = DB_Get_Connection()
    else:
        conn = connect
...
# ====== [DB] Drop Table Template ======= 
def DB_Drop_Table_Template(connect = None):
    if not connect:
        conn = DB_Get_Connection()
    else:
        conn = connect
...
# ====== [DB] Create Table Vip Code ===== 
def DB_Create_Table_VIP_Code(connect = None):
    if not connect:
        conn = DB_Get_Connection()
    else:
        conn = connect
...
# ====== [DB] Drop Table VIP Code ======= 
def DB_Drop_Table_VIP_Code(connect = None):
    if not connect:
        conn = DB_Get_Connection()
    else:
        conn = connect
    cursor = conn.cursor()
    cursor.execute("DROP TABLE IF EXISTS vip_codes")
    if not connect:
        conn.commit()
        conn.close()
...
# ====== [DB] Create Table Vip Variable = 
def DB_Create_Table_VIP_Variable(connect = None):
    if not connect:
        conn = DB_Get_Connection()
    else:
        conn = connect
...
# ====== [DB] Insert All VIP Variable === 
def DB_Save_All_VIP_Variable(data, connect = None):
    if not connect:
        conn = DB_Get_Connection()
    else:
        conn = connect
...
# ====== [DB] Get VIP Content ===========
def DB_Get_VIP_Variable(access_code: str):
    conn = DB_Get_Connection()
    cursor = conn.cursor()
...
# ====== [DB] Get All VIP Variables =====
def DB_Get_All_VIP_Variable():
    conn = DB_Get_Connection()
    cursor = conn.cursor()
...
# ====== [DB] Get Latest VIP Variable ===
def DB_Get_Latest_VIP_Variable():
    conn = DB_Get_Connection()
    cursor = conn.cursor()
...
# ====== [DB] Drop Table VIP Variable === 
def DB_Drop_Table_VIP_Variable(connect = None):
    if not connect:
        conn = DB_Get_Connection()
    else:
        conn = connect
...
def DB_Create_Table_Settings(connect = None):
    if not connect:
        conn = DB_Get_Connection()
    else:
        conn = connect
...
def DB_Drop_Table_Settings(connect = None):
    if not connect:
        conn = DB_Get_Connection()
    else:
        conn = connect
